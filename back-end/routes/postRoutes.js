...
const saveDetails = (app) => {
    ...
    app.post('/save_details' , async (req, res) => {
        let {firstname , surname , phone , email , age} = req.body

    try{
        let post = new postSchema ({
            firstname , surname , phone , email , age
        })
    ...
    app.delete('/delete_applicant/:id' , async (req , res)  =>  {
        try{
            const {id} = req.params  
            const partyToDelete = await postSchema.deleteOne({_id : id})
            res.send({message: "Successfully deleted", applicantToDelete})
        }catch(err){
            console.log(err);
            res.sendStatus(501)
        }
